...
namespace SmaliLib.Steps
{
    internal static class DepDownloader
    {
        ...
        public static void Download(IPlatform platform)
        {
            ...
            File.WriteAllBytes(Path.Combine("bin", "dexpatcher.jar"), platform.Download("https://github.com/DexPatcher/dexpatcher-tool/releases/download/v1.7.0/dexpatcher-1.7.0.jar", "dex bin"));
            if (PlatformCheck.IsWindows)
            {
                byte[] platformTools = platform.Download("http://dl.google.com/android/repository/platform-tools-latest-windows.zip", "adb");
            ...
            if (PlatformCheck.IsWindows)
                Unpack(platform.Download("https://github.com/fOmey/vdexExtractor/raw/master/bin/vdexExtractor_x86_64.zip", "vdex"));
            else
                platform.Log("Skipping vdexExtractor download as this is not windows, you may need to build it yourself: https://github.com/anestisb/vdexExtractor");
            Unpack(platform.Download("https://github.com/fOmey/compact_dex_converter/raw/master/compact_dex_converter_android_arm64-v8a.zip", "cdex"));
            File.WriteAllBytes(Path.Combine("bin", "module_installer.sh"), platform.Download("https://raw.githubusercontent.com/topjohnwu/Magisk/master/scripts/module_installer.sh", "moduleInstaller"));
        }
